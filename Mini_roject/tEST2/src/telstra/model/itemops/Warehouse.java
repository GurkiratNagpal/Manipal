...
//	}
	...
	public int[] getItemPrice() {
		...
		investment[0] = pd_list.stream().map(i->(i.getItemPrice()*i.getUnits())).reduce(0, (ans, un)->ans+un);
		//investment[1] = hcp_list.stream().map(i->(i.getItemPrice()*i.getUnits())).reduce(0, (ans, un)->ans+un);
		//investment[2] = ms_list.stream().map(i->(i.getItemPrice()*i.getUnits())).reduce(0, (ans, un)->ans+un);
		return investment;
		}
